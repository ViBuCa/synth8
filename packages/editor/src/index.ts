...
function installStyle() {
  ...
  element.textContent = `
    ...
    .s8-global-controls { display:grid; grid-template-columns:minmax(260px, .9fr) minmax(260px, 1.4fr); align-items:stretch; }
    .s8-global-left { display:flex; flex-direction:column; gap:8px; min-width:0; }
    ...
    .s8-song-controls > label { order:3; margin-left:0; }
    .s8-song-controls .s8-song-metrics { order:4; }
    .s8-global-controls > .s8-global-left { grid-column:1; }
    .s8-global-controls > .s8-track-panel { grid-column:2; grid-row:1; }
...
export function mountSynth8Editor(root: HTMLElement, options: EditorOptions = {}): { destroy(): void } {
  ...
  const render = () => {
    ...
    container.innerHTML = `<h2>Sketch Editor</h2><p>Choose a track type below. Click to add/remove. Drag melody notes to move them; drag their right edge to resize.</p>
      ...
      <div class="melody-section" ${editorTab === "melody" ? "" : "hidden"}><div class="s8-editor-toolbar s8-editor-melodies"><strong>Melodies</strong><div class="s8-track-list">${melodies.map((melody, index) => `<div class="s8-track-row"><button data-melody="${index}" class="${index === activeMelody ? "is-active" : ""}">${melody.name}</button><button data-melody-toggle="${index}" class="s8-track-toggle" title="${melody.enabled === false ? "Enable" : "Disable"} ${melody.name}">${melody.enabled === false ? "○" : "●"}</button></div>`).join("")}</div><div class="s8-track-actions"><button data-action="new-melody">+ New melody</button><button data-action="duplicate-melody">Duplicate</button><button data-action="delete-melody">Delete</button><label>Name <input data-melody-name value="${melodies[activeMelody].name.replace(/"/g, "&quot;")}" style="width:110px"></label></div></div>
      ...
      ${pitches.flatMap((midi) => Array.from({ length: Math.ceil(columns / gridStep) }, (_, column) => { const time = column * gridStep;
        ...
        return `<button class="s8-editor-cell ${keyClass} ${Math.abs(time % beatsPerBar) < 0.001 ? "beat bar" : ""} ${Math.floor(time) % 2 === 1 ? "beat-alt" : ""} ${note ? "note" : ""} ${start ? "note-start" : ""} ${end ? "note-end" : ""}" data-pitch="${midi}" data-start="${time}" data-duration="${note?.duration ?? 0}" aria-label="${noteName(midi)} beat ${time + gridStep}"></button>`;
    ...
    const trackPanel = container.querySelector<HTMLElement>("[data-track-panel]");
    const activeTrackToolbar = container.querySelector<HTMLElement>(editorTab === "melody" ? ".melody-section > .s8-editor-melodies" : ".drum-section > .s8-editor-melodies");
    if (trackPanel && activeTrackToolbar) trackPanel.appendChild(activeTrackToolbar);
